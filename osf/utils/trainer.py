...
class TrainingArguments:
    def __init__(
        ...
        self.smart_medium = smart_medium
...
class Trainer:
    ...
    def train(self):
        ...
        # read medium architectures from csv
        medium_archs = pd.read_csv("/lustre/hdd/LAS/jannesar-lab/msamani/OSF/refined_architectures_2var.csv")

        if self.args.reorder == 'once':
            if self.args.reorder_method == 'magnitude':
                self.supernet.mlp_layer_reordering()
            elif self.args.reorder_method == 'wanda':
                self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'wanda')
            elif self.args.reorder_method == 'gradient':
                self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'gradient')
            elif self.args.reorder_method == 'ricci':
                self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'ricci')
            elif self.args.reorder_method == 'hybrid':
                self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'hybrid')
            elif self.args.reorder_method == 'forman':
                self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'forman')
            elif self.args.reorder_method == 'wanda_ricci':
                mlp_scores = self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'wanda_ricci')
            elif self.args.reorder_method == 'gradient_ricci':
                self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'gradient_ricci')

        for epoch in range(self.args.num_train_epochs):
            if epoch > 0:
                if self.args.reorder == 'per_epoch':
                    if self.args.reorder_method == 'magnitude':
                        self.supernet.mlp_layer_reordering()
                    elif self.args.reorder_method == 'wanda':
                        self.supernet.mlp_layer_reordering(self.args.reorder_dataloader, 'wanda')
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                Num_random_subnets = 4
                for i in range(Num_random_subnets):
                    print("Random Subnet Training", i+1)

                    if self.args.smart_medium is False:
                        ...
                        # Train smart medium subnet
                        # pick random architecture from medium_archs
                        random_arch = medium_archs.sample(n=1).iloc[0]
                        (
                            self.activate_model,
                            self.activate_model.config.num_parameters,
                            self.activate_model.config.arch,
                        ) = self.supernet.smart_medium_model(sample_arch_config=random_arch)
                    ...
                    self.logger.log_metrics(train_metrics, step, prefix="steps/subnet/"+str(i+1))
                    self.logger.print_metrics(train_metrics, prefix="steps/subnet/"+str(i+1))
                    if (step + 1) % self.args.log_interval == 0:
                        metrics = self.evaluate(self.eval_dataloader)
                        self.logger.log_metrics(metrics, step, prefix="steps/subnet/"+str(i+1))
                        self.logger.print_metrics(metrics, prefix="steps/subnet/"+str(i+1))
