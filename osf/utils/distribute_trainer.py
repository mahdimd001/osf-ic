    ...
    def train(self, teacher_model=None):
        ...
        for epoch in range(self.args.num_train_epochs):
            ...
            for i, batch in enumerate(self.train_dataloader):
                ...
                step += 1
                if i ==5:
                    print(f"Rank {self.local_rank}: Breaking at batch {i}")
                    break
            ...
            # ============================================================
            # NEW CODE: INSERT HERE (Outside batch loop, Inside epoch loop)
            # ============================================================
            print(f"Rank {self.local_rank}: Reached end of batch loop. Waiting at barrier...")

            # 1. Synchronization Barrier
            dist.barrier()

            if self.local_rank == 0:
                print(f"Rank 0: Barrier passed. Checking reorder args: {self.args.reorder}")

            # 2. Reordering (Only on Rank 0)
            if self.local_rank == 0:
                if epoch >= 0:
                    if self.args.reorder == 'per_epoch':
                        print(f"Rank 0: Reordering supernet at end of epoch {epoch+1}...")
            ...
            # 4. Broadcast: Move to GPU first, then broadcast, then move back
            # NCCL requires tensors to be on the GPU
            self.supernet.model.to(self.device) 
            
            for param in self.supernet.model.parameters():
                dist.broadcast(param.data, src=0)
            
            # (Optional) Broadcast buffers if your reordering changes them (e.g., BN stats)
            for buffer in self.supernet.model.buffers():
                dist.broadcast(buffer.data, src=0)

            # Move back to CPU to match your existing code's behavior (accumulate_grad moves it to cpu)
            self.supernet.model.to("cpu")
